...
const labCss = (slug) =>
  [
    ...
    // Mirrors the workspace build's src/index.css (the base-owned entry that
    // pairs a kit's theme with the substrate's scroll-driven motion CSS) — the
    // kit's globals.css never imports across the overlay boundary, so the lab
    // has to make the same pairing itself, exactly as gen-kits.mjs does.
    '@import "../../kits/_base/src/motion/motion.css";',

...
A brutalist-modern design system for confident creative studios. The look is
loud and editorial: a bone off-white canvas, near-black ink, and a single
electric cobalt accent (`#2F4BFF`) used like a highlighter. Type does the
heavy lifting — oversized, tightly tracked display headlines set in Space
Grotesk, with monospace index labels and eyebrows in JetBrains Mono. Corners
are near-square (a `0.25rem` radius), borders are hairline rules, and there
are no soft shadows. The result reads as a studio with a point of view — the
deliberate opposite of a soft corporate SaaS page or a quiet warm portfolio.

This is a presentation system for an agency's own site, not a marketing
...
The chain is built to introduce a studio the way a confident studio would
introduce itself: voice first, proof second, invitation last. **Navbar**
fixes the wordmark and a single "start a project" action in reach at all
...
out. **Stats** turns track record into oversized animated numbers — a single
expansive beat after the denser grid and steps.

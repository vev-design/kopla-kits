...
> This guide covers **page sections** — the presentation flow. The **component catalog** (the reusable Button/Card/Badge/… primitives shown in the editor's Components view) has its own how-to: `AGENTS.components.md`.
>
> - **Building on a prebuilt kit?** The catalog is the kit's — leave it alone; just build/edit sections.
> - **Building from scratch (blank / no kit)?** You **own** the catalog. Don't lean on inline one-off markup for repeated UI: author each reusable primitive your sections use (button, card, badge, input, nav, …) as a real catalog component under `src/components/`, register it in `src/components/index.ts`, and **compose your sections from those components** — so the Components view reflects, and matches, the design you built. Follow `AGENTS.components.md`.
...
- **A CONTENT image is a block slot. A DESIGN element is a plain image prop.** Content is the section's own subject matter — a hero photograph, an image-text column, a case-study shot, a product screenshot, a gallery item. Type those as a union of block props from `@/components/blocks` and render with `<MediaBlock media={…} />`, unioning in the kinds that fit this section's design: `media: ImageBlockProps | VideoBlockProps` for a picture frame, and `| ChartBlockProps` as well where a data panel belongs. Design elements stay a plain `@kind image` string — logos and wordmarks, avatars and bylines, icons and glyphs, decorative textures and background patterns. Those are furniture the design brings, not content anybody swaps. When it is unclear, ask whether the section lays out a media FRAME for it (a column, a panel, a full-bleed band) or whether it is a small fixed-role mark sitting inside a text block: a frame is a slot.

  **This is the default, not a special case**, and the reason is what the two cost LATER rather than anything about today's design. Adding a kind to a slot that already exists is a non-breaking catalog change. Turning an `image: string` into a slot is a breaking prop shape, so every page already built against this system fails validation against the new catalog and needs a migration. A slot you did not strictly need costs nothing; a string you have to widen costs a migration. The old rule here asked you to use a slot only "when a media column could hold more than one kind of content", which meant predicting an edit somebody would want weeks later — so it read as false while writing almost every section, and across this kit set 4 of 91 sections ended up with a media slot while 22 typed their media as a string that can only ever be a picture.

  Blocks are pre-built, token-themed and static. Demo values for a slot are inline object literals (e.g. `media: { kind: 'chart', type: 'bar', data: [...] }`).
- **A video's PLAYBACK belongs to the block, not to your section.** `VideoBlockProps` carries `autoplay`, `muted`, `loop` and `hideControls` — four optional booleans, each OFF when omitted — and the host's inspector turns them into a switch apiece, recognised by those exact names. So render a clip through `<MediaBlock media={…} />` (or `<VideoBlock {...media} />`) and leave the element to the block: a `<video autoPlay muted loop>` written inside a section renders perfectly, publishes fine, and is invisible to the person editing the page, who then has to ask an agent for every change. Same for a hand-written YouTube or Vimeo `<iframe>` — the block already maps the four settings onto each provider's own parameters.

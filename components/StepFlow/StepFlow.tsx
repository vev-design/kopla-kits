...
// answer check per step and a score at the end.
//
// This component owns the MECHANICS and nothing about appearance: which step is
// showing, the progress indicator, where focus goes when the step changes, the
// announced position, locking an answer once it is given, and the score. What a
// step LOOKS like is the design's — pass the panels as `children` and this drives
// them without drawing their insides.
//
...
export function StepFlow({
  ...
  // Keyed by step index rather than an array so a `steps` prop that grew or
  // shrank cannot leave a stale slot behind.
  ...
  // replaced underneath it, which is a keyboard user reading nothing.
  ...
  const answer = (stepIndex: number, optionIndex: number) => {
    // Locked once given: a quiz whose answer can be changed after the feedback
    // appears is a quiz with a free retry, and its score means nothing.
    setAnswers((prev) => (stepIndex in prev ? prev : { ...prev, [stepIndex]: optionIndex }));
  };
  ...
  const score = steps.reduce((total, s, i) => {
    const picked = answers[i];
    return picked !== undefined && s.options?.[picked]?.correct ? total + 1 : total;
  }, 0);

  // A quiz can only advance once the question is answered; a stepper always can.
  const canAdvance = mode === 'quiz' ? answered : !isLast;
  const forwardLabel =
    nextLabel ?? (mode === 'quiz' && isLast ? 'See result' : 'Next');
  ...
  return (
    <div data-slot="step-flow" data-mode={mode} className={cn('flex flex-col gap-6', className)}>
      ...
      {steps.map((s, i) => {
        ...
        const isCurrent = interactive && i === current && !atResult;
        return (
          <div
            key={i}
            id={`${rootId}-step-${i + 1}`}
            data-slot="step-flow-panel"
            // Only the panel that is showing takes focus, and only once there is
            // more than one panel to be showing — an outline of every step needs
            // no tab stop of its own.
            ref={isCurrent ? panelRef : undefined}
          ...
          >
            ...
            {s.options && s.options.length > 0 && mode === 'quiz' ? (
              <fieldset
                ...
                // The platform locks the group, rather than this component
                // policing every change event: one attribute, and it disables
                // every control inside for free.
                      ...
                      picked === undefined
                        ...
                        : option.correct
                          ...
                          : picked === oi
                            ...
                            : undefined;
                    return (
                      // The verdict sits OUTSIDE the `<label>`, which is not a
                      // detail: a label wrapping an input IS that input's
                      // accessible name, so marking the row from inside renamed
                      // the control from "No" to "No, correct answer" the moment
                      // it was answered. A control whose name changes under the
                      // reader is the kind of thing that reads fine and tests
                      // false. Out here it is text beside the option instead.
                      <div
                        key={oi}
                        data-state={state}
                        className={cn(
                          'flex items-center gap-3 rounded-lg border pr-4 text-left text-sm',
                      ...
                      >
                        {/* 44px minimum, and on the label rather than the row:
                            the label is what receives the tap, and a
                            single-line answer would otherwise land near 32. */}
                        <label className="flex min-h-11 flex-1 cursor-pointer items-center gap-3 py-2.5 pl-4">
                          <input
                            type="radio"
                            name={`${rootId}-step-${i + 1}`}
                            value={oi}
                            checked={picked === oi}
                            onChange={() => answer(i, oi)}
                        ...
                        {state ? (
                          <>
                            ...
                            {/* The verdict in words, because a tick is not text. */}
      ...
      {atResult ? (
        <div
          ...
          className={cn('flex flex-col gap-3 rounded-lg border bg-card p-6 text-card-foreground', panelClassName)}
        >
      ...
      >
          ...
          <>
            <button
              ...
              onClick={() => go(step - 1)}
              disabled={current === 0}
              className="inline-flex min-h-11 items-center rounded-md border px-4 text-sm font-medium hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"
            >
              {backLabel}
            </button>
            <button
              ...
              onClick={() => go(step + 1)}
              disabled={!canAdvance}
              className="inline-flex min-h-11 items-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"
            >
              {forwardLabel}
            </button>
          </>
        )}
        {/* The visible counter IS the live region — one string, announced when it
            changes and readable at any time, instead of a drawn counter plus an
            sr-only twin saying the same thing. */}
